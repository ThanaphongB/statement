...
const dataTransaction=[
    {id:1,text:"เงินเดือน",amount:+15000},
    {id:2,text:"ค่าห้อง",amount:-2000},
    {id:3,text:"เที่ยว",amount:-2000},
...
function calculateMoney(){
   ...
   //คำนวณยอดคงเหลือ
    ...
    //คำนวณรายจ่าย
    const pay=amounts.filter(item=>item<0).reduce((result,item)=>(result+=item),0).toFixed(2);
    
    //เเสดงผลทางจอภาพ
    Balance.innerText=`฿`+formatNumber(total);
    money_plus.innerText=`฿`+formatNumber(income);
    money_minus.innerText=`฿`+formatNumber(pay);
}
